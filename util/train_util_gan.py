...
class TrainFramework():
    ...
    def train(self, epoch, D_G_opt_num=1, print_pre_step=100, test_pre_step=1000, save_pre_epoch=10, last_epoch=None, after_restore_func=None, after_valid_func=None, on_save_func=None, restore_func=None, epoch_test=True):
        '''
        ...
        for epoch in range(start_epoch, self.epoch+1):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                if not self.G_freeze:
                    self.optimizer_G.zero_grad()
                    loss[0].backward(retain_graph=True)
                    G_opt_num += 1

                if G_opt_num % D_G_opt_num == 0:
                    if not self.D_freeze:
