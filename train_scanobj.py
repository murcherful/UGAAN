...
parser.add_argument('--cate', default='chair', choices=['table', 'chair', 'sofa', 'bookshelf', 'bed', 'monitor', 'pillow', 'cabinet', 'bin', 'bag', 'box', 'desk', 'door', 'sink', 'toilet'], help='Category.')
parser.add_argument('--cuda_index', default='0', help='Use which GPU.')
args = parser.parse_args()

os.environ["CUDA_VISIBLE_DEVICES"] = args.cuda_index

cat_to_label = {'table':9, 'chair':4, 'sofa':13, 'bookshelf':8, 'bed':10, 'monitor':6, 'pillow':11, 
'cabinet':3, 'bin':1, 'bag':0, 'box':2, 'desk':5, 'door':7, 'sink':12, 'toilet':14}
...
from args_scanobj import ARGS_SCANOBJ
from train_util_gan import TrainFramework
from my_dataset import USGANDataset_ScanObj
from ugaan import UGAAN


def train(args):
    ARGS = ARGS_SCANOBJ(args.cate)
    train_dataset = USGANDataset_ScanObj(ARGS.lmdb_test, ARGS.lmdb_sn, ARGS.input_point_number, False, scanobj_obj=cat_to_label[args.cate], scanobj_par='test')
    train_dataset.sn.set_args(ARGS.min_scale, ARGS.max_scale, ARGS.max_center_shift)
    train_dataset.sn.pr = ARGS.pr
    valid_dataset = USGANDataset_ScanObj(ARGS.lmdb_test, ARGS.lmdb_sn, ARGS.input_point_number, False, scanobj_obj=cat_to_label[args.cate], scanobj_par='test')
